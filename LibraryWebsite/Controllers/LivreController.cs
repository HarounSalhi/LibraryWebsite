...
namespace Library.Controllers
{
    public class LivreController : Controller
    {
        ...
        [HttpPost]
        public ActionResult DownloadFile(Models.Livre model)
        {
            ...
            string path = Path.Combine(Server.MapPath("~/UploadedFiles/"), model.FileName);
            //System.IO.Path.ChangeExtension(path, null);
            byte[] bytes = System.IO.File.ReadAllBytes(path);

            //Send the File to Download.
            File(bytes, "application/octet-stream", model.FileName);
